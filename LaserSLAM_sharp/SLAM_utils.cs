...
class SLAM_utils
{
    ...
    public static int[] extract_sgrid(double[,] min_max_val, double pixelSize)
    {
        int[] sgrid = new int[2];
    ...
    public static int[] cal_idx(double[,] localMap, double[,] min_max_val, int[] sgrid, double pixelSize, int map_size)
    {
    ...
    public static double[,] ExtractLocalMap(double[,] map_points, double[,] scan, double[] pos)
    {
        ...
        double[,] scan_w = transform(scan, pos, scan.GetLength(0));
    ...
    public static double[,] double_pol2cart(LaserScan scan_f, LaserScan scan_r, double[,] p2c)
    {
        int scan_size_f = scan_f.LaserPoints.Count;
        for (int i = 0; i < scan_size_f; i++)
        {
            double angle = (double)scan_f.LaserPoints[i].Angle;
            double range = (double)scan_f.LaserPoints[i].Range;
            p2c[i, 0] = range * Math.Cos(angle);
            p2c[i, 1] = range * Math.Sin(angle);
        }
        int scan_size_r = scan_r.LaserPoints.Count;
        for (int i = 0; i < scan_size_r; i++)
        {
            double angle = (double)scan_r.LaserPoints[i].Angle;
            double range = (double)scan_r.LaserPoints[i].Range;
            p2c[i+ scan_size_f, 0] = range * Math.Cos(angle);
            p2c[i+ scan_size_f, 1] = range * Math.Sin(angle);
        }
        return p2c;
    }
    ...
    public static double[] diffPos(double[] prev_pos, double[] pos)
    {
    ...
    public static double[,] min_max_func(double[,] points, int scan_size)
    {
        double minX = 99999999, minY = 99999999, maxX = -99999999, maxY = -99999999;
        for (int i = 0; i < scan_size; i++)
        {
        ...
        double[,] min_max_value = new double[2, 2];
